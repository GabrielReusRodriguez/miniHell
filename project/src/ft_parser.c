...
/*   Updated: 2024/04/21 23:15:41 by gabriel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (token_set.last_read_token != NULL && token_set.last_read_token->next == NULL)
		return (FALSE);
	return (TRUE);
}
...
t_token	*ft_parser_get_exec(t_list **token_list_node, t_list *end_cmd)
{
	...
	//Caso que empiece por parantesis abierto. Habra que hacer un while para que salte
...
t_token *ft_parser_get_redir_input(t_list **token_list_node, t_list *end_cmd)
{
	...
	if (token->type == TOKEN_TYPE_RED_INPUT || token->type == TOKEN_TYPE_RED_HERE_DOC)
	{
		token_list_node = (*token_list_node)->next;
		return (token);
	}
...
void	*ft_parser_parse_command(t_cmd *cmd)
{
	...
	if (cmd->red_in != NULL)
		cmd->red_in_origin = ft_parser_get_redir_input_origin(&token_node, cmd->last_token);
	...
	if (cmd->red_out != NULL)
		cmd->red_out_dest = ft_parser_get_redir_output_dest(&token_node, cmd->last_token);
	return (NULL);
}